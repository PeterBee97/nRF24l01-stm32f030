...
uint8_t i=0,j;
extern uint8_t host_data[DATA_LENGTH],player_data[2][DATA_LENGTH];
...
{
	player_data[0][0]=0xF0 | 0x08 | (host_data[0]&0x03);
	player_data[1][0]=0xF0 | 0x04 | (host_data[0]&0x03);
...
{
	temp_data[i++]=data;
	if (i==DATA_LENGTH) i=0; 
	if (temp_data[i]==0x0A)
		if (temp_data[(i+DATA_LENGTH-1)%DATA_LENGTH]==0x0D)
		{
			for (j=0;j<DATA_LENGTH;j++)
				host_data[j]=temp_data[(i+j+1)%DATA_LENGTH];
				update_player_data();
			LED_ON;
		}
}
